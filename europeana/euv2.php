...
$_details = empty($_GET["details"]) ? 0 : $_GET["details"];
$page = new GoogleAnalytics\Page("/euv2.php");
$page->setTitle('Europeana v2 api');
$tracker->trackPageview($page, $session, $visitor);
$event = new GoogleAnalytics\Event("open-culture-app-v2",$action,$_details,0);
$tracker->trackEvent($event, $session, $visitor);
...
//		$item->originaltitle = $itm->title;
		...
		$query3="select * from musev2_eu_link where identifier = '" . $identifier .  "' order by type";
...
function english2($val) {
	...
	mysql_query("SET NAMES utf8", $db);
	mysql_query( "SET CHARACTER SET utf8", $db );
	$query="select * from musev2_eu_trans where txt = '". mysql_escape_string($val) ."';";
	error_log($query);
	...
	$query = ("insert into musev2_eu_trans (id,txt) values(0,'". mysql_escape_string($val) ."')");
	error_log($query);
	mysql_query($query);
	...
	foreach ($langs as $lang) {
		...
		if (count($data->data->translations) > 0) { 
			...
			$query = "update musev2_eu_trans set ".$lang." = '". mysql_escape_string($data->data->translations[0]->translatedText) ."' , detected_lang = '".$data->data->translations[0]->detectedSourceLanguage."' where id=".$newid;
			error_log($query);
...
function translateObject($key, $val, $target = "en") {
	...
	mysql_query("SET NAMES utf8", $db);
	mysql_query( "SET CHARACTER SET utf8", $db );
	$query="select * from musev2_eu_trans_object where _key = '$key';";
	error_log($query);
	$result=mysql_query($query);
	if (mysql_numrows($result) > 0) {
	...
	if (count($data->data->translations) > 0) {
		...
		$query = ("insert into musev2_eu_trans_object (id,_key,txt,detected_lang,translation) values(0,'".$key."','". mysql_escape_string($val) ."','".$dl."','".mysql_escape_string($newtext)."')");
		error_log($query);
		mysql_query($query);
...
function rijksmuseum_cloud_attribute($attribute, $value) {
	...
	if ($attribute == "publisher") {
		$query="select * from musev2_rijksmuseum_index where publisher = '" . $value .  "'";
		$result=mysql_query($query);
	...
	if ($attribute == "creator") {
		$query="select * from musev2_rijksmuseum_index where creator = '" . $value .  "'";
		$result=mysql_query($query);
		for ($r=0; $r < mysql_numrows($result); $r++) {
			echo "<li>&raquo; <a href='tes.php?action=linkcloud&identifier=".mysql_result($result,$r,"identifier")."'>".mysql_result($result,$r,"title")."</a></li>";
		}
	...
	if ($attribute == "type") {
		$query="select * from musev2_rijksmuseum_index where type = '" . $value .  "'";
		$result=mysql_query($query);
		for ($r=0; $r < mysql_numrows($result); $r++) {
			echo "<li>&raquo; <a href='tes.php?action=linkcloud&identifier=".mysql_result($result,$r,"identifier")."'>".mysql_result($result,$r,"title")."</a></li>";
		}
	...
	if ($attribute == "link") {
		$query="select * from musev2_rijksmuseum_link where url = '" . $value .  "' order by type";
...
function rijksmuseum_cloud($identifier) {
	...
	mysql_query("SET NAMES utf8", $db);
	mysql_query( "SET CHARACTER SET utf8", $db );
	$query="select * from musev2_rijksmuseum_index where identifier = '" . $identifier .  "'";
	$result=mysql_query($query);
	if (mysql_numrows($result) > 0) {
		...
		$query3="select * from musev2_rijksmuseum_link where identifier = '" . $identifier .  "' order by type";
		$result3=mysql_query($query3);
		for ($r=0; $r < mysql_numrows($result3); $r++) {
...
// https://www.rijksmuseum.nl/api/oai/68271713-6d46-4010-a3f9-d2ddb5890e1e/?verb=GetRecord&identifier=oai:rijksmuseum.nl/collection:COLLECT.5216&metadataPrefix=oaidc
	...
	$query999="INSERT INTO `musev2_rijksmuseum_tracking` (`id`, `type`, `sort`, `content`) VALUES ('0', 'item', 'get', '$identifier');";
...
function rijksmuseum_addlink($a, $b, $type, $comment) {
	...
	*/
	$s = "insert into musev2_eu_link  (id,ts,identifier,url,type,comment,uid) values (0,now() ";
	$s = $s . "," . Q($a);
	$s = $s . "," . Q($b);
	...
	$links = array();
		$query3="select * from musev2_eu_link where identifier = '" . ($a) .  "' order by type";
		$result3=mysql_query($query3);
		for ($r=0; $r < mysql_numrows($result3); $r++) {
...
function rijksmuseum_search($srch,$typ) {
	...
	*/
	$query999="INSERT INTO `musev2_rijksmuseum_tracking` (`id`, `type`, `sort`, `content`) VALUES ('0', 'term', 'srch', '$srch');";
	$result999=mysql_query($query999);

	$query="select * from musev2_rijksmuseum_index where description like '%" . $srch .  "%' or description_en like '%" . $srch .  "%' or title like '%" . $srch .  "%' or title_en like '%" . $srch .  "%' or creator like '%" . $srch .  "%' or subject like '%" . $srch .  "%' or publisher like '%" . $srch .  "%' or dat like '%" . $srch .  "%' ORDER BY (type != 'schilderij'),type,id limit 150";

	if ($typ && $typ != "") {
		$query="select * from musev2_rijksmuseum_index where (type = '".$typ."' or type_en = '".$typ."') and ( description like '%" . $srch .  "%' or description_en like '%" . $srch .  "%' or title like '%" . $srch .  "%' or title_en like '%" . $srch .  "%' or creator like '%" . $srch .  "%' or subject like '%" . $srch .  "%' or publisher like '%" . $srch .  "%' or dat like '%" . $srch .  "%') ORDER BY (type != 'schilderij'),type,id limit 150";
	}
	$result=mysql_query($query);
	// limit 3000
...
//		$totalitems = $items->items;
	...
	$query="select distinct type_en as itm from musev2_rijksmuseum_index where " . $_where;
	$result=mysql_query($query);
	$types = array();
	for ($r=0; $r < mysql_numrows($result); $r++) {
//		$item = "type:".mysql_result($result,$r,"itm");
	...
	$query="select distinct creator as itm from musev2_rijksmuseum_index where " . $_where;
	$result=mysql_query($query);
	$creators = array();
	for ($r=0; $r < mysql_numrows($result); $r++) {
		$item = mysql_result($result,$r,"itm");
	...
	$query="select distinct dat as itm from musev2_rijksmuseum_index where " . $_where;
	$result=mysql_query($query);
	$dats = array();
	for ($r=0; $r < mysql_numrows($result); $r++) {
		$item = mysql_result($result,$r,"itm");
