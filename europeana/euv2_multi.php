...
//http://europeana.eu/api/v2/record/92062/ED02C5CC49DEACF9A0B864E121D83955C355B971.json?wskey=ZHKKYAIMYT
	...
	mysql_query( "SET CHARACTER SET utf8", $db );
		...
		$query3="select * from muse_eu_link where identifier = '" . $identifier .  "' order by type";
...
function english2($val) {
	...
	$langs = array("en","fr","nl","de","pl","es","bg");
	$newtext = $val;
...
// https://www.rijksmuseum.nl/api/oai/68271713-6d46-4010-a3f9-d2ddb5890e1e/?verb=GetRecord&identifier=oai:rijksmuseum.nl/collection:COLLECT.5216&metadataPrefix=oaidc
	...
	if (mysql_numrows($result) > 0) {
		...
		if ($description != "" && $description_en == "") {
			...
			if ($description_en != "") {
				$replace_english_query = sprintf("update ignore muse_rijksmuseum_index set description_en = %s where identifier = '" . $identifier .  "'",QQ($description_en));
...
function rijksmuseum_addlink($a, $b, $type, $comment) {
	...
	*/
	$s = "insert into muse_eu_link  (id,ts,identifier,url,type,comment,uid) values (0,now() ";
	$s = $s . "," . Q($a);
	$s = $s . "," . Q($b);
	...
	$links = array();
		$query3="select * from muse_eu_link where identifier = '" . ($a) .  "' order by type";
		$result3=mysql_query($query3);
		for ($r=0; $r < mysql_numrows($result3); $r++) {
