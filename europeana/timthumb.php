...
function tim($S) {
	...
	global $ltim;
	$ntim = microtime(true);
	error_log(sprintf("timthumb timer : step: %s  total: %s  - (%s)", ($ntim - $ltim), ($ntim - $tim),$S));
	$ltim = $ntim;
}
tim("start");
...
if(! defined('ERROR_IMAGE') )			define ('ERROR_IMAGE', './10x10.gif');									// Image to serve if an error occurs instead of showing error message 
...
class timthumb {
	...
	public static function start(){
		...
		if($tim->tryBrowserCache()){
			tim("start - setep1 - exit");
			exit(0);
		}
		$tim->handleErrors();
		if(FILE_CACHE_ENABLED && $tim->tryServerCache()){
			tim("start - setep1 - exit 2");
			exit(0);
		}
		$tim->handleErrors();
		tim("start - setep pre run");
		$tim->run();
		tim("start - setep post run");
		$tim->handleErrors();
		exit(0);
	}
	...
	public function run(){
		if($this->isURL){
			...
			if($this->param('webshot')){
				...
				tim("run - externalimage start");
				$this->debug(3, "webshot is NOT set so we're going to try to fetch a regular image.");
				$this->serveExternalImage();
				tim("run - externalimage end");

			}
		} else {
			tim("run - internalimage start");
			$this->debug(3, "Got request for internal image. Starting serveInternalImage()");
			$this->serveInternalImage();
			tim("run - internalimage end");
		}
		return true;
	}
			...
			if($this->isURL){
				...
				if(filesize($this->cachefile) < 1){
					...
					if(time() - @filemtime($this->cachefile) > WAIT_BETWEEN_FETCH_ERRORS){
						...
						$this->set404();
						$this->error("An error occured fetching image."  .$this->cachefile . " / ");
						return false; 
					}
				}
			} else {
